...
class StarPlanVoteDialog extends Component {
  constructor(props){
    super(props);
    const { t, onClose, onSure, bxhAsset, balance} = this.props;
  ...
  renderCard = () => {
    ...
    const { sureEnable, balance, inputVal, bxhAsset} = this.state;
    const isMobile = this._isMobile();
    return (
        <Card className={classes.card}>
            ...
            <div className={classes.cardTipInput}>
                ...
                <label><span style={{color:'rgba(255,255,255,0.4)',marginRight:'5px'}}>{t('BXH.availableNumber')}:</span>{balance?SaveToTwoWei(balance,4):"--"}</label>
            </div>
...
export default withNamespaces()(withRouter(withStyles(styles)(StarPlanVoteDialog)));

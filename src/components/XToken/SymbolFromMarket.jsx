...
class SymbolFromMarket extends Component {
    constructor(props) {
        super(props)
    ...
    render() {
        ...
        // console.log("symbolList1111=>", symbolList)
    ...
    renderListItem = (obj,idx) => {
        ...
        const symbol = (obj&&obj.symbol1)||"--";
        const symbolLog = (obj&&obj.symbolPair)||"--";
        // console.log('fronSymbol===>', toSymbol)
        
        return (
            <label key={idx}>
                <div className={ classes.syslist }>
                    <div className={ classes.sysicoimg }>
                        <img src={getTokenLogoURLWithName(symbolLog)} /> {symbol}
                    </div>

                    {
                        obj.symbolPair === toSymbol.symbolPair ?
                        <div className={getStyleClass('xdtbrilabel',classes.sysicovalue)} onClick={() => { this.onCheck(obj) }}>
                            <span>{isNoEmpty(obj.XTokenBalance)?obj.XTokenBalance:'--'}</span>
...
export default withNamespaces()(withRouter(withStyles(styles)(SymbolFromMarket)));

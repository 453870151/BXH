...
class StarPlanDialog extends Component {
  constructor(props){
    super(props);
  ...
  viewProjectWebsite = () => {
      window.open(this.state.obj.site_url)
  }
  render() {
            ...
            <Card className={classes.card} onClick={(e) => {e.stopPropagation();}}>
                ...
                <div className={classes.content}>
                    <div className={classes.header}>
                        <img src={obj.icon_url ? obj.icon_url : ''} alt='' />
                        <span>{obj.project_name}</span>
                    </div>
                    <div className={classes.desc}>{obj.info}</div>
                    <div className={classes.line}></div>
                    <div className={classes.info}>
                        <div className={classes.row}>
                            <div>{t('BXH.releaseTime')}</div>
                            <span>{moment(obj.release_time*1000).format('YYYY/MM/DD HH:mm')}</span>
                        </div>
                        <div className={classes.row}>
                            <div>{t('BXH.maximumSupply')}</div>
                            <span>{toShowDollar(SaveToTwoWei(obj.max_amount,2))} {obj.token_name}</span>
                        </div>
                        <div className={classes.row}>
                            <div>{t('BXH.currentSupply')}</div>
                            <span>{toShowDollar(SaveToTwoWei(obj.cur_amount,2))} {obj.token_name}</span>
                        </div>
                        <div className={classes.row}>
                            <div>{t('BXH.belongsMaleChain')}</div>
                            <span>HECO</span>
                        </div>
...
export default withNamespaces()(withRouter(withStyles(styles)(StarPlanDialog)));

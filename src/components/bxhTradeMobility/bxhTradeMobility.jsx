...
class BxhTradeMobility extends Component {
        ...
        <div className={getStyleClass('PCbroot',classes.root)}>
          ...
          <div className={classes.root1}>
            <div className={classes.bxhtTit}>
              ...
              <h3>{t('BXH.tigongtip')} {pair && pair.symbolPair && pair.symbolPair !== "" ? pair.symbolPair : ""} {t('BXH.zhuanqubxh')}</h3>
            </div>
  ...
  MAXBalance = () => {
    const balance = this.state.tokensData.mineLpAmount;
  ...
  refreshCount = (count) => {
    ...
    var val = _getValuemultip1(this.state.tokensData.mineLpAmount, temp_count)//parseFloat(this.state.tokensData.mineLpAmount) * (count / 100)
    ...
    return (
      <div>
              ...
              <em>
                {
                  rewardBXHFactory && rewardBXHFactory[0].tokens[0].shouyi ?
                    numberDecimal(rewardBXHFactory[0].tokens[0].shouyi)
                    :
                    0.00
                }
              </em>
        ...
        <div className={classes.bxhbotomflex}>
          {
            RemoveLiquidityAllowanceAmount > 0 ?
              <div className={getStyleClass('PC_new_btn2',classes.bxhclickbottm, classes.bxhflexman)} style={{ marginRight: '0px' }} onClick={() => { this.onRemoveLiquidity() }}>{t('BXH.jianshaoliudong')}</div>
              :
              <div className={getStyleClass('PC_new_btn2',classes.bxhclickbottm, classes.bxhflexman)} style={{ marginRight: '0px' }} onClick={() => { this.onAllowanceRemoveLiquidity() }}>{approve1Title}</div>//approve0Title
          }
  ...
  nav = () => {
    this.props.history.push('/bxhTradeStake/' + this.state.pair.id)
  }
  ...
  sendWithdrawSY = () => {
    ...
    if (rewardBXHFactory && rewardBXHFactory[0].tokens[0].shouyi) {
      msg = "Claim " + numberDecimal(parseFloat(rewardBXHFactory[0].tokens[0].shouyi)) + " BXH"
    } else {
      msg = "Claim 0 BXH"
    }
    this.setState({
...
export default withNamespaces()(withRouter(withStyles(styles)(BxhTradeMobility)));

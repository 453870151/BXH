...
class Twist extends Component {
    ...
    componentDidMount() {
        ...
        //通过id获取币对及详情
        dispatcher.dispatch({ type: GET_TWISTPOOLINFOBYID, content: { id: this.state.match.params.id } })
    }
    ...
    componentWillMount() {
        ...
        emitter.on(GET_TWISTPOOLINFOBYID_RETURNED, this.getPoolItemInfo)
    ...
    componentWillUnmount() {
        ...
        emitter.removeListener(GET_TWISTPOOLINFOBYID_RETURNED, this.getPoolItemInfo)
        ...
        return (
            <div className={classes.header}>
                <div className={classes.headerTitle}>
                    X{pool ? pool.symbol0 : null} Pool
                </div>
                <div className={classes.headerDesc}>
                    {t('BXH.shoudaocunru')} {pool ? pool.symbol0 : null} {t('BXH.twget')} X{pool ? pool.symbol0 : null},{t('BXH.pledge')} X{pool ? pool.symbol0 : null} {t('DFKII.earn')} BXH
                </div>
            </div>
        )
    }
    ...
    renderDeposit() {
        ...
        let chainID = localStorage.getItem('chainIDSwitch')
        let isDisableDeposit = true;
        // console.log('poolpoolpoolpool====>', pool)
        if(chainID === '56'){
            if(pool) {// 0x开头禁用存入
                isDisableDeposit = pool.symbolPair_Show.indexOf('0x')===0;
            }
        }else{
            isDisableDeposit = false;
        }
        ...
        return (
            <div className={getStyleClass('PCTDaoCard',classes.bxhDaoCard)}>
                ...
                <div className={classes.bxhshuom}>
                    Deposit {pool ? pool.symbol0 : null} harvest X{pool ? pool.symbol0 : null}
                </div>
                {
                    rewardBXHFactory ?
                        <div>
                            {
                                rewardBXHFactory && rewardBXHFactory[0].tokens[0].alloWance0 !== 0 ?
                                    ...
                                    <div className={classes.rewardCardBtnContentRow}>
                                        <div onClick={() => { if(isDisableDeposit===false){this.openMortgageBack('0')} }} className={getStyleClass('PC_new_btn1',classes.rewardCardPledge)} style={{filter:`grayscale(${isDisableDeposit?1:0})`}}>{t('BXH.shoudaocunru')}</div>
                                        {
                                            chainID === '56' ?
    ...
    renderPledge() {
        ...
        return (
            <div className={getStyleClass('PCTDaoCard',classes.bxhDaoCard)}>
                ...
                <div className={classes.bxhshuom}>
                    X{pool ? pool.symbol0 : null} Tokens Staked
                </div>
                {
    ...
    renderReceive() {
        ...
        return (
            <div className={getStyleClass('PCTDaoCard',classes.bxhDaoCard)}>
                ...
                <div className={classes.bxhsumzhi}>
                    {rewardBXHFactory && rewardBXHFactory[0].tokens[0].shouyi > 0.0001 ? numberDecimal(parseFloat(rewardBXHFactory[0].tokens[0].shouyi))||"--" : "0.0000"}
                </div>
                <div className={classes.bxhshuom}>Deposit Tokens Earned BXH</div>
    ...
    twistAlloWance1 = () => {
        const { pool } = this.state
        this.setState({ modalSend: false })
        const msgContent = "Approve X" + pool.symbol0;
        this.setState({ modalSend: true, modalSendType: 0, msgContent: msgContent })
...
export default withNamespaces()(withRouter(withStyles(styles)(Twist)));